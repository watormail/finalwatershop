 ...
 * 版权所有，侵权必究！
 */
package com.etc.config.handler;
...
public class TokenToMallUserMethodArgumentResolver implements HandlerMethodArgumentResolver {
    ...
    @Autowired
    private UserDao mallUserDao;
    @Autowired
    private UserTokenDao mallUserTokenDao;
    ...
    public Object resolveArgument(MethodParameter parameter, ModelAndViewContainer mavContainer, NativeWebRequest webRequest, WebDataBinderFactory binderFactory) {
        if (parameter.getParameterAnnotation(TokenToMallUser.class) instanceof TokenToMallUser) {
            ...
            if (null != token && !"".equals(token) && token.length() == Constants.TOKEN_LENGTH) {
                MallUserToken mallUserToken = mallUserTokenDao.findByToken(token);
                ...
                mallUser = mallUserDao.findByUserId(mallUserToken.getUserId());
                if (mallUser == null) {
