...
import com.etc.api.vo.NewBeeMallOrderDetailVO;
import com.etc.api.vo.NewBeeMallOrderItemVO;
import com.etc.api.vo.NewBeeMallOrderListVO;
...
public class OrderServiceImpl implements OrderService {
    ...
    @Autowired
    private OrderDao newBeeMallOrderMapper;
    @Autowired
    private OrderItemDao newBeeMallOrderItemMapper;
    @Autowired
    private ShoppingCartItemDao newBeeMallShoppingCartItemMapper;
    @Autowired
    private GoodsDao newBeeMallGoodsMapper;
    @Autowired
    private OrderAddressDao newBeeMallOrderAddressMapper;
    @Autowired
    private StockNumDTODao stockNumDTOMapper;

    @Override
    public NewBeeMallOrderDetailVO getOrderDetailByOrderNo(String orderNo, Long userId) {
        NewBeeMallOrder newBeeMallOrder = newBeeMallOrderMapper.findByOrOrderNo(orderNo);
        ...
        List<NewBeeMallOrderItem> orderItems = newBeeMallOrderItemMapper.findByOrderId(newBeeMallOrder.getOrderId());
        //获取订单项数据
    ...
    public PageResult getMyOrders(PageQueryUtil pageUtil) {
        List<NewBeeMallOrder> newBeeMallOrders = newBeeMallOrderMapper.queryOrder(pageUtil);
        ...
        if (total > 0) {
            ...
            if (!CollectionUtils.isEmpty(orderIds)) {
                ...
                for (int i=0;i<orderIds.size();i++){
                    orderItems.addAll(newBeeMallOrderItemMapper.findByOrderId(orderIds.get(i)));
                }
    ...
    public String cancelOrder(String orderNo, Long userId) {
        ...
        if (newBeeMallOrder != null) {
            ...
            //todo 订单状态判断
            newBeeMallOrder.setOrderStatus((byte)NewBeeMallOrderStatusEnum.ORDER_CLOSED_BY_MALLUSER.getOrderStatus());
            if (newBeeMallOrderMapper.save(newBeeMallOrder)!=null) {
    ...
    public String saveOrder(MallUser loginMallUser, MallUserAddress address, List<NewBeeMallShoppingCartItemVO> myShoppingCartItems) {
        ...
        for (int i=0;i<goodsIds.size();i++){
            newBeeMallGoods.add(newBeeMallGoodsMapper.findById(goodsIds.get(i)).get());
        }
        //检查是否包含已下架商品
        ...
        if (!CollectionUtils.isEmpty(itemIdList) && !CollectionUtils.isEmpty(goodsIds) && !CollectionUtils.isEmpty(newBeeMallGoods)) {
            for(int i=0;i<itemIdList.size();i++){
                NewBeeMallShoppingCartItem shoppingCartItem=newBeeMallShoppingCartItemMapper.findById(itemIdList.get(i)).get();
                shoppingCartItem.setIsDeleted((byte)1);
            }
            if (itemIdList.size()>0) {
                List<StockNumDTO> stockNumDTOS = BeanUtil.copyList(myShoppingCartItems, StockNumDTO.class);
                stockNumDTOMapper.saveAll(stockNumDTOS);
                ...
                if (newBeeMallOrderMapper.save(newBeeMallOrder) !=null) {
                    ...
                    //保存至数据库
                    if (newBeeMallOrderItemMapper.saveAll(newBeeMallOrderItems).size() >0 && newBeeMallOrderAddressMapper.save(newBeeMallOrderAddress)!=null) {
